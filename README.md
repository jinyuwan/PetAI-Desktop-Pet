...
**自动适配**：`states` 里未配置的标准状态（idle / thinking / reading / planning / working / task_done / work_done），应用会自动从骨架的动画名中模糊匹配最接近的（精确 → 包含 → 关键词），找不到则保持待机，不会报错。所以**只写 `idle` 也能用**，想姿势丰富再补齐映射。
